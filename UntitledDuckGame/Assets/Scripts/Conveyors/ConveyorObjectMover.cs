...
public class ConveyorObjectMover : MonoBehaviour {
    ...
    private void Update() {
        ...
        //determine if object is currently pickup up
        ...
        if (wasPickupActive) {
            wasPickupActive = false;
            if (rb != null) {
                // stay kinematic under conveyor control to avoid post-release jitter
                rb.isKinematic = true;
                rb.useGravity = false;
            }
    ...
    private void ReleaseFromConveyor() {
        ...
        if (tan.sqrMagnitude <= 1e-6f) {
            ...
            if (fallbackTan.sqrMagnitude > 1e-6f) {
                tan = fallbackTan;
            }
            else {
