...
| **Left mouse (hold)** | Grab a `Pickup` / operate an `Operate` interactable under the cursor. | `CursorController` → `IsometricRaycaster.BeginHold/EndHold` |
...
`mouth` transform by `quackRotation`. `ToggleMouth` is used by `PickupInteractable` to "hold"
items in the mouth.
...
- `HandleVerticalIK()` — maps mouse **Y** within `±innerZoneRangeY` to a target reach height
  (`min/maxIKY`), lerped onto `ik_target.y`. Calls `HandleVerticalIKSweep()` (currently a
  **scaffold/stub** with detailed comments but no active collision response).
...
`tbikc` (TwoBoneIKConstraint), `armObjects[]`, `playerHoldSlot` (where picked-up items reparent),
and three blocking `LayerMask`s.
...
## Gotchas
- **Pickup/drop is buggy** and on the fix list — see [interactables-items-conveyors.md](interactables-items-conveyors.md)
  and [status-roadmap.md](status-roadmap.md).
